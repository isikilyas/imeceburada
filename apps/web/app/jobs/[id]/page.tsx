...
import { EMPLOYMENT_TYPES, JobPostingDto, LISTING_INTENTS, TRADE_CATEGORIES } from "@bau360/shared";
...
export default function JobDetailPage() {
  ...
  const listingTypeLabel = LISTING_INTENTS.find((l) => l.value === job.listingType)?.label ?? job.listingType;
  ...
  return (
    <div className="mx-auto max-w-2xl">
      ...
      <p className="mt-1 text-silver-500">{job.companyName}</p>
      ...
      <div className="mt-8">
        ...
        {applyError && <p className="mt-2 text-sm text-red-400">{applyError}</p>}
      </div>
    </div>
  );
}
