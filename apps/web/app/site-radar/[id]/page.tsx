...
export default function SiteRequestDetailPage() {
  ...
  if (!request) return <p className="text-silver-500">Çağrı bulunamadı.</p>;
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="flex items-start justify-between gap-4">
        ...
        <WhatsAppShareButton
          text={`🚨 Anlık Çağrı: ${request.title} — ${request.city} (${request.neededCount} kişi gerekiyor)\nİmece Yapı'da incele:`}
      ...
      <p className="mt-1 text-silver-500">
        {request.createdByName} · {request.city} · {request.neededCount} adet gerekiyor
      </p>
      {request.neededBy && (
        <p className="mt-1 text-sm text-silver-500">
          Son tarih: {new Date(request.neededBy).toLocaleString("tr-TR")}
        </p>
      )}
      ...
      {isOwner ? (
        <div className="mt-8">
          <Link href={`/site-radar/${request.id}/responses`} className="text-gold-400 hover:underline">
            Gelen yanıtları gör ({request.responseCount})
          </Link>
        </div>
      ) : (
        <div className="mt-8">
          {!authLoading && !user && <p className="text-sm text-silver-500">Yanıt vermek için giriş yapmalısın.</p>}
          {user && status !== "done" && (
            <div className="space-y-3">
              ...
              >
                {status === "submitting" ? "Gönderiliyor..." : "Yanıt Ver"}
              </button>
            </div>
          )}
          {status === "done" && <p className="text-sm text-green-400">Yanıtın gönderildi!</p>}
