...
export default function NewSiteRequestPage() {
  ...
  async function handleSubmit(e: FormEvent) {
    e.preventDefault();
    if (!location) {
      setError("Lütfen haritadan şantiye konumunu işaretle");
      return;
    }
    setError(null);
    ...
    try {
      ...
      setError(err instanceof ApiError ? err.message : "Çağrı oluşturulamadı");
    } finally {
  ...
  if (!user) return <p className="text-silver-500">Çağrı açmak için giriş yapmalısın.</p>;

  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-6 text-2xl font-semibold text-silver-300">Anlık Çağrı Aç</h1>
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <Field label="Başlık">
          <input
            required
            placeholder="Örn: Bu akşama kadar 3 kalıpçı lazım"
            value={form.title}
        ...
        <Field label="Konum (haritada tıklayarak işaretle)">
          <SiteMap
            markers={[]}
        ...
        >
          {isSubmitting ? "Yayınlanıyor..." : "Çağrıyı Yayınla"}
        </button>
      </form>
    </div>
  );
