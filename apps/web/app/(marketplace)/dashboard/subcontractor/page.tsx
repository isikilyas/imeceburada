...
import { ApplicationDto } from "@imeceburada/shared";
...
export default function SubcontractorDashboardPage() {
  ...
  const { data: applications, isLoading } = useQuery({
    ...
    enabled: user?.role === "SUBCONTRACTOR",
  });
  ...
  return (
    <div className="grid grid-cols-1 gap-10 lg:grid-cols-2">
      <section>
        <h1 className="mb-6 text-2xl font-semibold text-silver-300">
          {t("dashboard.subcontractor.applicationsHeading")}
        </h1>
        {isLoading && <ListSkeleton count={3} />}
        {applications?.length === 0 && (
          <p className="text-silver-500">
            {t("dashboard.subcontractor.noApplications")}{" "}
            <Link href="/jobs?listingType=SUBCONTRACTOR" className="text-gold-400 hover:underline">
              {t("dashboard.subcontractor.browseJobsLink")}
            </Link>
          </p>
        )}
              ...
              <Link href={`/jobs/${app.jobId}`} className="min-w-0 flex-1">
                ...
                {app.offeredWage != null && (
                  <p className="mt-1 text-xs text-gold-400">
                    Firmanın teklifi: {app.offeredWage.toLocaleString("tr-TR")} ₺
                  </p>
                )}
              </Link>
      ...
      <section>
        <h2 className="mb-6 text-2xl font-semibold text-silver-300">{t("dashboard.subcontractor.profileHeading")}</h2>
        <Link
          href="/account"
