...
export default function MySiteRequestsPage() {
  ...
  return (
    <div>
      <h1 className="mb-6 text-2xl font-semibold text-silver-300">{t("siteRadar.mine.heading")}</h1>
      {isLoading && <p className="text-silver-500">{t("common.loading")}</p>}
      {requests?.length === 0 && <p className="text-silver-500">{t("siteRadar.mine.empty")}</p>}
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-silver-500">
                    {kindLabel} · {r.city} · {r.status === "OPEN" ? t("siteRadar.mine.statusOpen") : t("siteRadar.mine.statusClosed")}
                  </p>
                </div>
                <div className="flex shrink-0 gap-3 text-sm">
                  <Link href={`/site-radar/${r.id}/responses`} className="text-gold-400 hover:underline">
                    {t("siteRadar.mine.responseCount", { count: r.responseCount })}
                  </Link>
                  {r.status === "OPEN" && (
                    <button onClick={() => handleClose(r.id)} className="text-silver-500 hover:text-red-400">
                      {t("siteRadar.mine.close")}
                    </button>
                  )}
                </div>
              </div>
