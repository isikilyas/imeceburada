...
function SearchResults() {
  ...
  return (
    <div>
      <h1 className="mb-2 text-2xl font-semibold text-silver-300">{t("search.heading")}</h1>
      <p className="mb-6 text-sm text-silver-500">
        {q ? t("search.resultsFor", { q }) : t("search.promptEmpty")}
      </p>
      ...
      {q && isLoading && <p className="text-silver-500">{t("search.searching")}</p>}
      {q && !isLoading && totalResults === 0 && <p className="text-silver-500">{t("common.noResults")}</p>}

      {!!jobs?.items.length && (
        <section className="mb-8">
          <h2 className="mb-3 text-lg font-medium text-silver-300">{t("search.jobsHeading", { count: jobs.items.length })}</h2>
      ...
      {!!equipment?.items.length && (
        <section className="mb-8">
          <h2 className="mb-3 text-lg font-medium text-silver-300">{t("search.equipmentHeading", { count: equipment.items.length })}</h2>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
      ...
      {!!materials?.items.length && (
        <section className="mb-8">
          <h2 className="mb-3 text-lg font-medium text-silver-300">{t("search.materialsHeading", { count: materials.items.length })}</h2>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
